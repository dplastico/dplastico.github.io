...
Setup and Banner functions are just to prepare the binary for the challenge (buffering and printing the banner) So lets examine this functions, lets focus on **missile_launcher()** sin the **show_missiles()** function seems to only print and no aparent format string vulnerability.
...
Above is the Pseudo Code from the disassenbly of the **missile_launcher()** function shown us a bug that allow us to leak an address from the binary code, how? if you check this lines in detail you can understand it.
...
With all this information exploitation becomes trivial, as I mentioned at the begingin of the post I already explained this in several post so I will just explain a brief summary on how I will proceed to exploit it, this is just a regular ROP.
...
- Idenetify necesary 
- Use the puts function to print a GOT Address and leak a libc address (GOT asddress as argument of puts)
...
This will inmediatly provide us a clue on how to exploit the binary. Let's examine the option 2 "Buy Weapons"
...
def leak_1(data):
    ...
    return leak
```
I didn'r endedup using this stack leak, but I keep it just in case there's a notehr way to exploit it.
...
Option "3" let us create a chunk on the heap, but this will alocate a new chunk and storage will continue to point to the same chunk. Opiton "4" it's interesting because it will allow us to to free the storage.
...
The following exploit acomplish this
...
Also since I did some writeups of heap challenges before I will skip some basic details of the techniques that will be used, but if you are not familiar with heap exploiation you can read the following articles before if you like:
...
You can reverse engineering the whole binary (it's not buig) but I will focus on the functions and sections that will lead us to exploitation. Below you can see the pseudocode from the main function
...
As you can see it looks like a typycal Heap challenge. The option "1"  will allow us to allocate a chunk on the heap ad fill it with data, option "2" to read from a chunk allocated, option "3" to edit a chunk on the heap,  option "4" will free a chunk on the heap that we indicate trough the index number, and option "5" will exit the program
...
We can see on the "if" statement that checks that the index is no less tahn 0 and more than 20 (you are limited to 20 allocations). but it also check whethre the pointer on the heap represented by *"(_QWORD *)(8LL * num + a1)"*.
...
```c
    ...
    read(0, *(void **)(8LL * num + a1), v1);
```
As you can see v1 will hold the value returned by the lenght of the string on the heap. and then that value is used by read as the size of data to write on the same heap chunk
...
so as shown above first we generated a 0x20 size chunk (0x18, will gave you a 0x20 chunk, a 0x28 a 0x30 one, and so on...) follow by a 0x430 size chunk that once freed will not be direct to the tcache  <LINK ACA>. Then we allocate three more  0x20 chunks another 0x430 and a guard chunk to avoid consolidation with the top chunk
...
edit(a, p64(0)*3+p8(0x90)) # calculated 0x490 size to acoomodate for the fake large chunk
delete(e)
```
...
Now with a leak and since we still have a chunk in the unsorted bin, we can easily trigger a tcache poison situation. Freeing the **chunk d** adn then requesting a chunk that will overlap **chunk d** so we can fill the tcache pointer to the next ecache bin in this case an address of our choose. and Sicne wea re in libc version 2.27 we dont need to care about the key field or the count on the tcache. We can acomplish this with the following code
...
delete(e)
#
#this generate a larhe 0x8a0 chunk in the unsorted bin
f = make(0x428, "F")
#libc leak
leak = show(c)
...
I really enjoyd the pwn challenges on this CTF, I would love to have more time to finish  all of them and also to maybe do some RE challs. but maybe next time, you can contact me at @dplastico for any feedback.
